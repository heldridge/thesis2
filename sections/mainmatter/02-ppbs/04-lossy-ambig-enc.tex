...
\begin{definition}[Weak Lossy Ambiguity]
	Let $\mathsf{PKE} = (\setup,$ $\kgen, \kgenlossy, \enc, \dec)$ be a lossy public-key encryption scheme for message space $\mathcal{M}$ and let the random variable $\wlambgame_b(\mathsf{PKE}, \adv, \lambda)$, where $\adv = \set{\adv_1, \adv_1}$, $b \in\{0,1\}$, and $\secpar \in \NN$ denote the result of the following probabilistic experiment:
...
We now show that \sla{} is strictly stronger than \wla{}.

\begin{theorem}
	Strong-lossy-ambiguity is strictly stronger than \wla{}.
\end{theorem}
