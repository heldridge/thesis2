...
\begin{proof}
	...
	\begin{itemize}
		...
		\item $\nexthyb$: Replace $\otpct_d^{\mat}$ with an element sampled uniformly at random from $\msgspace_\mat$.
		      ...
		      $\bdv$ runs $\adv$, receives $(\thr, r_{KG}, pk = (pk', C_{KG}, \pi_{KG}), x)$, and outputs $(\allowbreak(\allowbreak pk',\allowbreak C_{KG}),\allowbreak \pi_{KG})$.
		      ...
		      As $\nu_2(\secpar)$ is negligible, we can assume that there exists an extractor $\Ext$ that given $\pi_{KG}$ outputs $(\thr', sk', r_k, r_c)$ such that $C_{KG} = \commit(\compp, \thr; r_c)$ and $(sk',\allowbreak pk')\allowbreak = \allowbreak\kgen'(\allowbreak\secpar, \allowbreak\pkepp^{\rev}, \pkepp^{\mat}, \base, \numdigits, \thr; r_k)$.
		      ...
		      $\bdv$ runs $\adv$, receives $(\thr, r_{KG}, pk = \allowbreak(pk', \allowbreak C_{KG}, \allowbreak\pi_{KG}),\allowbreak x)$, and uses $\Ext$ to extract $(\thr, sk', r_k, r_c)$.
