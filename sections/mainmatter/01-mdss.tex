...
Our correctness definition differs from standard secret sharing in that we must reconstruct secrets given a set of shares from \emph{multiple} dealers. This set may include shares from both sufficient dealers (i.e. those outputting at least $\mdssRecThr$ shares) and \emph{insufficient} dealers (i.e. those outputting fewer than $\mdssRecThr$ shares).
We require that $\mdssReconstructAlg$ outputs exactly the set of secrets shared by the sufficient dealers, except with some failure probability $\mdssCVar$.
Similar to standard secret sharing, we only consider correctness for honestly generated (i.e. via the $\mdssShareAlg$ algorithm) sets of shares.
	...
	\begin{proof}
		...
		\begin{pcvstack}[center, space=1em]
			\procedure{$\bdv^j_1(\secparam)$}{
				...
				\pcreturn (\mdssSecret_j, \mdssSecret', \mdssNumShares_{\mdssRelBatch, 0}, 1, \indexSet_{\mdssRelBatch, 0}[\mdssIWRB], 1), (\st, \mdssNumShares_{\mdssRelBatch, 0}, \set{\mdssShareSet_i}_{i \in [\mdssRelBatch - 1]}, \set{\mdssShareSet_i}_{i \in [x+1, \mdssNumShareSets]})
			}

			\procedure{$\bdv_2^j((\st, \mdssNumShares_{\mdssRelBatch, 0}, \set{\mdssShareSet_i}_{i \in [\mdssRelBatch - 1]}, \set{\mdssShareSet_i}_{i \in [x+1, \mdssNumShareSets]}), \mdssBlindShare_1, \dots, \mdssBlindShare_{\mdssIWRB})$}{
				\forall k \in [\mdssIWRB + 1, \mdssNumShares_{\mdssRelBatch, 0}]: \mdssBlindShare_k \gets \mdssSampleShare(\secparam) \\
				\mdssShareSet_x := \set{\mdssBlindShare_k}_{k \in [\mdssNumShares_{\mdssRelBatch, 0}]} \\
		...
		$\bdv^j$ then invokes $\adv$ on the aggregated share sets and outputs what $\adv$ outputs.
