...
\begin{definition}[One-More Unlinkability]
	Let $(\mdss = (\mdssShareAlg, \mdssReconstructAlg)$ be an MDSS scheme and let the random vaiable $\omulink_b(\mdss, \adv, \secpar)$ where $\adv = \set{\adv_1, \adv_2}$, $b \in \bin$, and $\secpar \in \NN$ denote the result of the following experiment:
	\procedureblock{$\omulink_b(\pke, \adv, \secpar)$}{
		...
		b' \gets \adv_2(\st, \mdssShare{\mdssSecret_0}_1, \dots, \mdssShare{\mdssSecret_0}_{\mdssNumShares_0 - 1}, \mdssShare{\mdssSecret_b}^*)
	}
	We say that an adversary $\adv$ is \emph{admissible} if it is the case that $|\indexSet| \leq \mdssPrivThr$.

	The $\omulink(\mdss, \adv, \secpar)$ advantage of $\adv$ is defined as
	\begin{align*}
		 & \advantage{\omulink}{\adv, \mdss} =                                                           \\
		 & \abs{\prob{\omulink_0(\pke, \adv, \secpar) = 1} - \prob{\omulink_1(\pke, \adv, \secpar) = 1}}
	\end{align*}
	We say that an MDSS scheme is $\mdssULVar$-one-more-unlinkable if for all admissible NUPPT adversaries $\adv$ we have that $\advantage{\omulink}{\adv, \mdss} \leq \mdssULVar(\secpar)$.

\end{definition}

\begin{definition}[MD-Correctness]
	\label{def:md-correctness}
